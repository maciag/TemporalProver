...
StringFormula::decomposeType FormulaNode::decomposeNext(
	...
	if (!allChecked && !eliminated) {
		...
		for (int i = 0; i < formulas.size(); i++) {
			if (!isChecked[i]) {
				if (formulas[i].getType()
                        == StringFormula::stacking) {
					stackIdx = i;
				} else if (formulas[i].getType()
                        == StringFormula::single) {
					singleIdx = i;
				} else if (formulas[i].getType()
                        == StringFormula::branching_stacking) {
					stackBranchIdx = i;
				} else if (formulas[i].getType()
                        == StringFormula::branching_substacking) {
					substackBranchIdx = i;
				} else if (formulas[i].getType()
                        == StringFormula::branching) {
					branchIdx = i;
				} else if (formulas[i].getType()
                        == StringFormula::next) {
					nextIdx = i;
		...
		if (singleIdx >= 0) {
            ...
            return StringFormula::branching_stacking;
            ...
            return StringFormula::branching_substacking;
		} else if (branchIdx >= 0) {
			childNodes.clear();
			childNodes.resize(2);
			vector<StringFormula> subformulas;
            ...
            return StringFormula::branching;
		} else if (nextIdx >= 0) {
			childNodes.clear();
			childNodes.resize(1);
